...
for lang in ('zh','en'):
    ...
    assert len(defs) == 79 and len(set(defs)) == 79
...
for lang,path in [('zh','_posts/2026-08-25-no-view-is-the-whole.md'),('en','en/posts/no-view-is-the-whole.md')]:
    ...
    assert len(re.findall(r'^### ',post,re.M)) == 15
    assert len(re.findall(r'^## ',post,re.M)) == 7
    assert len(re.findall(r'^\[\^[^\]]+\]:',post,re.M)) == 79
...
print('PASS: all 17 source files align; complete rendered prose; 79 notes, 17 images and 5 tables per language; valid links and article headings.')
print(stats)
